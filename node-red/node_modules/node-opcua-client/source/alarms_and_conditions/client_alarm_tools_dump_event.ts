 ...
 * @param eventFields
 */
export async function dumpEvent(session: IBasicSession, fields: string[], eventFields: Variant[]) {
    ...
    async function __dumpEvent1(
        ...
        if (variant.dataType === DataType.NodeId) {
            ...
            console.log(chalk.yellow(w(name, 30), w(_fields[index], 25)),
                chalk.cyan(w(DataType[variant.dataType], 10).toString()), chalk.cyan.bold(name), "(", w(variant.value, 20), ")");

        } else {
            // tslint:disable-next-line: no-console
            console.log(chalk.yellow(w("", 30), w(_fields[index], 25)),
